...
=== Registries, Membership and Identity

Registries are used in the Spatial Web to recognize membership in a DOMAIN.
...
This clause includes elements of a proposal to the IEEE Registration Advisory Committee (SWF IEEE RAC proposal_V3_2.14.2025)
...
MLB is a Registrant, Domain Authority, Registry, Registrar, and Index—all at
once. It controls its ecosystem, issues SWID, credentials for Subdomains and
enables structured search within its domain, while remaining connected to the
broader UDG. One registration (one SWID), many discoverable contexts
