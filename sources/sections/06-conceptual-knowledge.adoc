...
The viewpoint in this clause considers the UDG as a knowledge graph (KG). A KG is a representation of information that uses a graph-structured data model to represent and operate on data. KGs are used for interlinked descriptions of entities, e.g., linked data.

The truth of knowledge in the Spatial Web refects the reality of truth in other mediums.  Contradictory statements will exist across the KG.  Knowledge in the Spatial Web will be both statement of fact and probablistic beliefs.  Knowledge will change through a dialectic process of social agents as defined in <<section-conceputal-view-social-network>>.

The Spatial Web Universal Domain Graph (UDG) KG is a graph where the nodes are Spatial Web ENTITIES.  The interlinking for ENTTIES in the UDG is created by: 1. links between ENTITIES or 2. ENTITIES being members of a DOMAIN entity.
...
Note that common shapes can be bound specifically to reference domains and included or imported, as specified in the section link:#importing-taxonomies-and-schemas[Importing Taxonomies and Schemas].
